            ...
            pagesPromises.push(
              pdf.getPage(i)
                .then((page) => page.getTextContent()
                  .then((textContentPage) => textContentPage.items.map((item) => item.str).join(' ')),
                ),
            );
          }

          Promise.all(pagesPromises).then((pagesTextArray) => {
...
const extractAcceptanceDetails = (text) => {
  ...
  if (!text.toUpperCase().includes('OFFER OF ADMISSION')) {
    alert('Please upload an Admission Letter');
    return { results: [] };
  }
  ...
  if (coopRecommendationMatch) {
    ...
    if (text.includes('Extended Credit Program')) {
      details.extendedCreditProgram = 'yes';
      details.coopProgram = 'No';
    } else if (text.includes('Co-op Program')) {
      details.coopProgram = 'Yes';
  ...
  details.exemptionsCourses = extractCoursesFromText({
    text,
  ...
  details.deficienciesCourses = extractCoursesFromText({
    text,
  ...
  details.transferCredits = extractCoursesFromText({
    text,
...
function extractCoursesFromText({ text, startLabel, endLabel, courseType, results }) {
