...
interface CourseSectionButtonProps {
  title: string;
  hidden: boolean;
}

interface GroupedSections {
  [termCode: string]: Section[];
}

const CourseSectionButton = ({ title, hidden }: CourseSectionButtonProps) => {
  ...
  const groupByTerm = (sections: Section[]): GroupedSections => {
    return sections.reduce((acc: GroupedSections, section: Section) => {
  ...
  const translateTermCode = (termCode: string): string => {
    const seasonCodes: { [key: string]: string } = {
      '1': 'Summer',
      '2': 'Fall',
      '3': 'Fall/Winter',
      '4': 'Winter',
  ...
  const formatEnrollment = (section: Section): string => {
    return (
      `${section.currentEnrollment}/${section.enrollmentCapacity}` +
      (section.waitlistCapacity > 0 ? ` (Waitlist: ${section.currentWaitlistTotal}/${section.waitlistCapacity})` : '')
    );
  };
  ...
  const fetchSections = async (): Promise<void> => {
    ...
    try {
      ...
      const data: Section[] = await api.get(`/section/schedule?subject=${subject}&catalog=${catalog}`, {
        signal: abortController.signal,
      });

      const filtered = data.filter((section: Section) => {
          ...
          <div className="modal-card">
            ...
            {loading ? (
              ...
              <>
                ...
                {!loading && sections.length === 0 && !error && <p>No upcoming sections found</p>}
