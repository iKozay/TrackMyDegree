...
describe('DegreeAuditService', () => {
  ...
  describe('capstone / ENGR 490 exemption', () => {
    it('should exempt ENGR 490 when program capstone is completed', async () => {
      // Add ENGR 490 to the DB and a pool that includes it
      await Course.create({
        _id: 'ENGR 490',
        title: 'Capstone Design Project',
        credits: 4,
        description: 'General engineering capstone',
      });
      await CoursePool.create({
        _id: 'ENGR_REQ',
      ...
      await Degree.findByIdAndUpdate('COMP', {
        coursePools: ['CORE_CS', 'MATH_REQ', 'CAPSTONE', 'GEN_ED', 'ENGR_REQ'],
      });

      // Student completed COMP 490 (program capstone)
      await Timeline.findByIdAndUpdate(testTimelineId, {
        courseStatusMap: new Map([
          ['COMP 248', { status: 'completed', semester: 'Fall 2022' }],
          ['COMP 490', { status: 'completed', semester: 'Fall 2024' }],
        ]),
      });
      ...
      const engrReq = audit.requirements.find(r =>
        r.title.toLowerCase().includes('engineering'),
      );
      expect(engrReq).toBeDefined();

      const engr490 = engrReq?.courses.find(c => c.code === 'ENGR 490');
      expect(engr490).toBeDefined();
      expect(engr490?.status).toBe('Completed');
      expect(engrReq?.creditsCompleted).toBe(4);
    });

    it('should exempt ENGR 490 when program capstone is in progress', async () => {
      await Course.create({
        _id: 'ENGR 490',
        title: 'Capstone Design Project',
        credits: 4,
      ...
      await Timeline.findByIdAndUpdate(testTimelineId, {
        courseStatusMap: new Map([
          ['COMP 490', { status: 'inprogress', semester: 'Winter 2025' }],
        ]),
      });
    ...
    it('should NOT exempt ENGR 490 when capstone is not started', async () => {
      await Course.create({
        _id: 'ENGR 490',
        title: 'Capstone Design Project',
        credits: 4,
      ...
      // No capstone course in the status map
      await Timeline.findByIdAndUpdate(testTimelineId, {
        courseStatusMap: new Map([
          ['COMP 248', { status: 'completed', semester: 'Fall 2022' }],
        ]),
      ...
      const engr490 = engrReq?.courses.find(c => c.code === 'ENGR 490');
      expect(engr490?.status).toBe(STATUS_MISSING);
    });

    it('should also exempt ENGR 490 when using generateDegreeAuditFromTimeline', () => {
      const mockDegree = {
        _id: 'COMP',
        name: DEGREE_NAME,
        totalCredits: 120,
      ...
      const mockSemesters = [
        { term: 'Fall 2024', courses: [{ code: 'COMP 490' }] },
      ];
      ...
      const mockCourses: Record<string, TimelineCourse> = {
        'COMP 490': {
          ...
          offeredIn: [],
          rules: [],
          status: { status: 'completed', semester: 'Fall 2024' },
        },
        'ENGR 490': {
          id: 'ENGR 490',
          title: 'Capstone Design Project',
