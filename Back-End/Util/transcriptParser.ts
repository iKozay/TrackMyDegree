...
export class TranscriptParser {
  ...
  private extractStructuredLines(pdfData: any): string[] {
    ...
    allTextItems.forEach((item) => {
      ...
      const shouldBreakLine =
        (Math.abs(item.y - lastY) > Y_THRESHOLD || item.page !== lastPage) && currentLine.length > 0;
  ...
  private extractStudentInfo(lines: string[]): StudentInfo {
    ...
    for (let i = 0; i < expandedLines.length; i++) {
      ...
      if (
        (line.includes(', QC') || line.includes(', ON') || line.includes(', BC')) &&
        !line.includes('Canada') &&
        !line.includes('|') &&
        line.length < 100
      ) {
    ...
    for (let i = 0; i < expandedLines.length; i++) {
      ...
      if (line === 'Active in Program') {
        ...
        for (let j = i + 2; j < Math.min(i + 10, expandedLines.length); j++) {
          if (expandedLines[j].includes('Bachelor of') || expandedLines[j].includes('Master of')) {
            ...
            admitTerm = j + 1 < expandedLines.length ? expandedLines[j + 1] : '';
          } else if (
          ...
          } else if (expandedLines[j] === 'Active in Program' || expandedLines[j].includes('Min. Credits Required')) {
            break;
          }
        }
  ...
  private parseCoursePdf2jsonData(texts: any[], startIndex: number): TranscriptCourse | null {
    ...
    const section = decodeURIComponent(texts[startIndex + 2].R?.[0]?.T || '').trim();
    ...
    const grade = decodeURIComponent(texts[titleIndex].R?.[0]?.T || '').toUpperCase();
    titleIndex++;
    ...
    if (grade !== 'PASS' && grade !== 'EX' && grade !== '0.00' && grade !== '0' && grade !== '0.0') {
      ...
      while (
        ...
        !['WKRT', 'RPT', 'EX'].includes(decodeURIComponent(texts[titleIndex].R?.[0]?.T || ''))
      ) {
        titleIndex++;
      }
    } else {
