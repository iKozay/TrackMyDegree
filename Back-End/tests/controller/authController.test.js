...
const authController = require('../../dist/controllers/authController/authController_mongo').default;
...
describe('Auth Controller', () => {
  ...
  test.each(userTypes)('should register a new user', async (type) => {
    ...
    const result = await authController.registerUser({ fullname, email, password, type });
  ...
  test('should not allow weak password on registration', async () => {
    const fullname = faker.person.firstName() + ' ' + faker.person.lastName();
    const email = faker.internet.email();
    const password = faker.internet.password({length: 7, memorable: true, pattern:/[A-Za-z\d@$!%*?&]/});
    const type = 'student';
  ...
  test('should authenticate user login', async () => {
    ...
    const regResult = await authController.registerUser({ fullname, email, password, type });
  ...
  test('should not authenticate with wrong password', async () => {
    ...
    const badAuth = await authController.authenticate(email, 'wrongPassword123!');
    expect(badAuth).toBeUndefined();
    });

  test('should not authenticate non-existent user', async () => {
    const authResult = await authController.authenticate('nonexistent', 'somePassword123!');
  ...
  test('should handle database error during authentication', async () => {
    ...
    const result = await authController.authenticate('test@example.com', 'somePassword123!');
  ...
  test('should reset password for existing user', async () => {
    ...
    const reuseResult = await authController.resetPassword(otp, 'AnotherPass123!', 'AnotherPass123!');
  ...
  test('should not reset password with invalid OTP', async () => {
    const resetResult = await authController.resetPassword('nonexistentotp', 'NewPass123!', 'NewPass123!');
    expect(resetResult).toBeUndefined();
  });
  
  test('should not reset password when new password does not match confirmation password', async () => {
    const resetResult = await authController.resetPassword('otp', 'NewPass123!', 'NewPass12345!');
    expect(resetResult).toBeUndefined();
  });
  ...
  test('should not reset passsword if otp is missing', async () => {
    const resetResult = await authController.resetPassword(null, 'NewPass123!', 'NewPass123!');
    expect(resetResult).toBeUndefined();
  });
  ...
  test('should handle database error during resetPassword', async () => {
    ...
    const result = await authController.resetPassword('1234', 'NewPass123!', 'NewPass123!');
  ...
  test('only admin users are recognized as admins', async () => {
    const adminFullname = faker.person.firstName() + ' ' + faker.person.lastName();
    const adminEmail = faker.internet.email();
    const adminPassword = generateStrongPassword();
    const adminType = 'admin';

    const studentFullname = faker.person.firstName() + ' ' + faker.person.lastName();
    const studentEmail = faker.internet.email();
    const studentPassword = generateStrongPassword();
    const studentType = 'student';

    const adminReg = await authController.registerUser({ fullname: adminFullname, email: adminEmail, password: adminPassword, type: adminType });
    expect(adminReg).toHaveProperty('id');
    const adminUser = await User.findById(adminReg.id);
    expect(adminUser).toBeTruthy();
    expect(adminUser.type).toBe('admin');

    const studentReg = await authController.registerUser({ fullname: studentFullname, email: studentEmail, password: studentPassword, type: studentType });
  ...
  test('should handle bcrypt error during password reset', async () => {
    ...
    const newPassword = generateStrongPassword();
    const result = await authController.resetPassword('1234', newPassword, newPassword);
