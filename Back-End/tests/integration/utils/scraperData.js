...
async function getScraperData() {
  ...
  try {
    ...
    console.error(`[getScraperData] Failed to create temp dir ${DEFAULT_TEMP_DIR}: ${e.message}`);
    throw e;
  }
  ...
  await Promise.all(
    ...
    degreeNames.map(async (degreeName) => {
      try {
        ...
        console.error(`[getScraperData] Error getting scrapper data for "${degreeName}": ${e.message}`);
      }
    })
  );

  if (written.length > 0) {
    console.log(`[getScraperData] Written ${written.length}/${degreeNames.length} files`);
  }
...
async function cleanupScraperFiles() {
  try {
    ...
    console.error(`[cleanupScraperFiles] Unexpected removal error for ${DEFAULT_TEMP_DIR}: ${e.message}`);
  }
}
