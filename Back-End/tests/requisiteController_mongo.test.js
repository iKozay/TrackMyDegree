...
describe('RequisiteController Mongoose', () => {
  ...
  describe('createRequisite', () => {
    ...
    it('should create a prerequisite successfully', async () => {
      ...
      const result = await requisiteController.createRequisite('SOEN490', 'SOEN390', 'pre');
    ...
    it('should create a corequisite successfully', async () => {
      ...
      const result = await requisiteController.createRequisite('SOEN490', 'SOEN390', 'co');
    ...
    it('should throw error if course1 does not exist', async () => {
      await Course.create({ _id: 'SOEN390', title: 'Software Engineering', credits: 3, description: 'SE course', prerequisites: [], corequisites: [] });

      await expect(requisiteController.createRequisite('NONEXISTENT', 'SOEN390', 'pre'))
        .rejects.toThrow("One or both courses ('NONEXISTENT', 'SOEN390') do not exist.");
    });

    it('should throw error if course2 does not exist', async () => {
      await Course.create({ _id: 'SOEN490', title: 'Capstone', credits: 4, description: 'Final project', prerequisites: [], corequisites: [] });

      await expect(requisiteController.createRequisite('SOEN490', 'NONEXISTENT', 'pre'))
        .rejects.toThrow("One or both courses ('SOEN490', 'NONEXISTENT') do not exist.");
    });

    it('should throw error if requisite already exists', async () => {
      await Course.create([
        { _id: 'SOEN490', title: 'Capstone', credits: 4, description: 'Final project', prerequisites: ['SOEN390'], corequisites: [] },
        { _id: 'SOEN390', title: 'Software Engineering', credits: 3, description: 'SE course', prerequisites: [], corequisites: [] }
      ]);

      await expect(requisiteController.createRequisite('SOEN490', 'SOEN390', 'pre'))
        .rejects.toThrow('Requisite with this combination of courses already exists.');
    });
  });
  ...
  describe('readRequisite', () => {
    ...
    beforeEach(async () => {
      await Course.create([
        { _id: 'SOEN490', title: 'Capstone', credits: 4, description: 'Final project', prerequisites: ['SOEN390', 'SOEN287'], corequisites: ['SOEN491'] },
        { _id: 'SOEN390', title: 'Software Engineering', credits: 3, description: 'SE course', prerequisites: [], corequisites: [] },
        { _id: 'SOEN287', title: 'Web Programming', credits: 3, description: 'Web course', prerequisites: [], corequisites: [] },
        { _id: 'SOEN491', title: 'Capstone 2', credits: 4, description: 'Final project 2', prerequisites: [], corequisites: [] }
      ]);
    });
    ...
    it('should return all requisites for a course', async () => {
      ...
      expect(result.some(r => r.code2 === 'SOEN390' && r.type === 'pre')).toBe(true);
      expect(result.some(r => r.code2 === 'SOEN287' && r.type === 'pre')).toBe(true);
      expect(result.some(r => r.code2 === 'SOEN491' && r.type === 'co')).toBe(true);
    });

    it('should return specific requisite when code2 and type provided', async () => {
      const result = await requisiteController.readRequisite('SOEN490', 'SOEN390', 'pre');
    ...
    it('should return empty array when specific requisite does not exist', async () => {
      const result = await requisiteController.readRequisite('SOEN490', 'NONEXISTENT', 'pre');

      expect(result).toHaveLength(0);
    });

    it('should throw error if course does not exist', async () => {
      await expect(requisiteController.readRequisite('NONEXISTENT'))
  ...
  describe('updateRequisite', () => {
    ...
    it('should update requisite successfully', async () => {
      ...
      const result = await requisiteController.updateRequisite('SOEN490', 'SOEN390', 'pre');
    ...
    it('should throw error if course1 does not exist', async () => {
      ...
      await expect(requisiteController.updateRequisite('NONEXISTENT', 'SOEN390', 'pre'))
        .rejects.toThrow("One or both courses ('NONEXISTENT', 'SOEN390') do not exist.");
    });
  ...
  describe('deleteRequisite', () => {
    ...
    beforeEach(async () => {
      await Course.create([
        { _id: 'SOEN490', title: 'Capstone', credits: 4, description: 'Final project', prerequisites: ['SOEN390'], corequisites: ['SOEN491'] },
        { _id: 'SOEN390', title: 'Software Engineering', credits: 3, description: 'SE course', prerequisites: [], corequisites: [] },
        { _id: 'SOEN491', title: 'Capstone 2', credits: 4, description: 'Final project 2', prerequisites: [], corequisites: [] }
      ]);
    });
    ...
    it('should delete prerequisite successfully', async () => {
      const result = await requisiteController.deleteRequisite('SOEN490', 'SOEN390', 'pre');

      expect(result).toBe('Requisite with the course combination provided has been successfully deleted.');
    ...
    it('should delete corequisite successfully', async () => {
      const result = await requisiteController.deleteRequisite('SOEN490', 'SOEN491', 'co');
    ...
    it('should throw error if course does not exist', async () => {
      await expect(requisiteController.deleteRequisite('NONEXISTENT', 'SOEN390', 'pre'))
        .rejects.toThrow('Course does not exist.');
    });

    it('should throw error if requisite does not exist', async () => {
      await expect(requisiteController.deleteRequisite('SOEN490', 'NONEXISTENT', 'pre'))
