...
describe('degreeControllerMongo (integration with in-memory MongoDB)', () => {
  ...
  describe('createDegree', () => {
    it('should create and return a degree if id and name are unique', async () => {
      ...
      const dbDegree = await Degree.findOne({ _id: 'CS' }).lean();
    ...
    it('should throw an error if degree with same id or name already exists', async () => {
      // seed degree
      await new Degree({ _id: 'CS', name: 'Computer Science', totalCredits: 120 }).save();
    ...
    it('should throw an error if degree with same name but different id already exists', async () => {
      // seed degree
      await new Degree({ _id: 'CS', name: 'Computer Science', totalCredits: 120 }).save();

      await expect(
        degreeController.createDegree('CS2', 'Computer Science', 120)
      ).rejects.toThrow('Degree with this id or name already exists.');
    });

    it('should create degree with different name and id', async () => {
      // seed degree
      await new Degree({ _id: 'CS', name: 'Computer Science', totalCredits: 120 }).save();

      const result = await degreeController.createDegree('SE', 'Software Engineering', 130);
      expect(result).toHaveProperty('id', 'SE');
      expect(result).toHaveProperty('name', 'Software Engineering');
      expect(result).toHaveProperty('totalCredits', 130);
    });
  });
  ...
  describe('readAllDegrees', () => {
    it('should return a list of all degrees except ECP', async () => {
      await Degree.insertMany([
        ...
        { _id: 'ECP', name: 'ECP Degree', totalCredits: 30 }
      ]);
    ...
    it('should return empty array when only ECP degree exists', async () => {
      await new Degree({ _id: 'ECP', name: 'ECP Degree', totalCredits: 30 }).save();
  ...
  describe('updateDegree', () => {
    ...
    it('should update only name when credits remain the same', async () => {
      await new Degree({ _id: 'CS', name: 'Computer Science', totalCredits: 120 }).save();

      const result = await degreeController.updateDegree('CS', 'Computer Science Updated', 120);
