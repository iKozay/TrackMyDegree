...
async function authenticate(
  ...
  return undefined; // always return undefined for invalid login credentials
    ...
    Sentry.captureException({ error: 'Backend error - authenticate (mongo)', details: error });
...
async function registerUser(
  ...
  try {
   ...
   await newUser.save();
    ...
    Sentry.captureException({ error: 'Backend error - register user (mongo)', details: error });
...
async function isAdmin(user_id: string): Promise<boolean | undefined> {
  try {
    ...
    Sentry.captureException({ error: 'Backend error - isAdmin (mongo)', details: error });
