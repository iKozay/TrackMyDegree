...
const DBController_1 = __importDefault(require("../DBController/DBController")); // Your database connection utility
...
const createBackup = async (req, res, next) => {
    ...
    `;
        ...
        await fs_2.promises.writeFile(backupFilePath, JSON.stringify(backupData, null, 2), 'utf-8');
        res.status(200).json({
            success: true,
...
const listBackups = async (req, res, next) => {
    try {
        ...
        const files = await fs_2.promises.readdir(backupDir);
...
const restoreBackup = async (req, res, next) => {
    try {
        ...
        const backupContent = await fs_2.promises.readFile(backupFilePath, 'utf-8');
...
const reseedData = async (res) => {
    ...
    const newPool = await DBController_1.default.getConnection();
    if (!newPool) {
        res.status(500).json({ success: false, message: DB_CONNECTION_FAILED });
        return;
    }
};
const insertBackupData = async (newPool, filteredBackupData, res) => {
    // ========================================================
    ...
    try {
        for (const tableName of Object.keys(filteredBackupData)) {
            ...
            for (const record of records) {
                ...
                try {
                    await insertRequest.query(`INSERT INTO [${tableName}] (${columnsJoined}) VALUES (${placeholders})`);
                }
                catch (error) {
...
const deleteBackup = async (req, res, next) => {
    try {
        ...
        await fs_2.promises.unlink(backupFilePath);
        res
            .status(200)
...
async function generateNextId(t, tableName, prefix) {
...
async function upsertDegree(t, id, name, totalCredits, isAddon) {
    const request = new sql.Request(t);
...
async function upsertCoursePool(t, poolName) {
    const request = new sql.Request(t);
...
async function upsertCourse(t, code, title, credits, description, offeredIn) {
    const request = new sql.Request(t);
...
async function upsertDegreeXCoursePool(t, degreeId, poolId, creditsRequired) {
    const request = new sql.Request(t);
...
async function upsertCourseXCoursePool(t, courseCode, poolId, groupId) {
    const request = new sql.Request(t);
...
async function upsertRequisite(t, requisite) {
...
async function getPoolIdByName(t, poolName) {
    const request = new sql.Request(t);
    request.input('name', sql.VarChar, poolName);
    const result = await request.query('SELECT id FROM CoursePool WHERE name = @name');
    if (result.recordset.length === 0) {
