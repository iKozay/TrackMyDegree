...
exports.getTableRecords = exports.getTables = exports.deleteBackup = exports.restoreBackup = exports.listBackups = exports.createBackup = void 0;
exports.seedSoenDegree = seedSoenDegree;
const DBController_1 = __importDefault(require("@controllers/DBController/DBController")); // Your database connection utility
...
const getBackupDir = () => {
    return process.env.BACKUP_DIR || path_1.default.join(__dirname, '../../backups');
};
/**
 * Create a full JSON backup of all allowed tables. Each tables data dumped into one big JSON file with timestamp.
 */
const createBackup = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const pool = yield DBController_1.default.getConnection();
        if (!pool) {
            res
                .status(500)
    ...
    `;
        const tableResult = yield pool.request().query(tableQuery);
        const tables = tableResult.recordset.map((row) => row.TABLE_NAME);
        // Create an object mapping table names to their records
        const backupData = {};
        for (const tableName of tables) {
            const result = yield pool.request().query(`SELECT * FROM [${tableName}]`);
        ...
        yield fs_2.promises.writeFile(backupFilePath, JSON.stringify(backupData, null, 2), 'utf-8');
        res.status(200).json({
            success: true,
 ...
 */
const listBackups = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const backupDir = getBackupDir();
        ...
        const files = yield fs_2.promises.readdir(backupDir);
 ...
 */
const restoreBackup = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const backupDir = getBackupDir();
        const backupFilePath = path_1.default.join(backupDir, backupName);
        ...
        const backupContent = yield fs_2.promises.readFile(backupFilePath, 'utf-8');
        ...
        yield deletionTx.begin();
        try {
            for (const tableName of allTablesReversed) {
                yield deletionTx.request().query(`DELETE FROM [${tableName}]`);
            }
            yield deletionTx.commit();
            console.log('Allowed tables deleted successfully.');
        }
        catch (deleteError) {
            yield deletionTx.rollback();
            throw deleteError;
        }
        // ========================================================
        ...
        yield seedSoenDegree();
        console.log('Database reseeded successfully.');
        const newPool = yield DBController_1.default.getConnection();
        if (!newPool) {
            res
                .status(500)
                .json({ success: false, message: 'Database connection failed' });
        ...
        yield insertTx.begin();
        try {
            for (const tableName of Object.keys(filteredBackupData)) {
                // Delete any leftover records (just in case)
                yield insertTx.request().query(`DELETE FROM [${tableName}]`);
                const records = filteredBackupData[tableName];
                if (records.length === 0)
                    continue;
                for (const record of records) {
                    ...
                    try {
                        yield insertRequest.query(`INSERT INTO [${tableName}] (${columnsJoined}) VALUES (${placeholders})`);
                    }
                    catch (error) {
                        if (error instanceof Error && error.message.includes('FOREIGN KEY constraint')) {
                            console.error(`Foreign key constraint error while inserting into table "${tableName}". Record:`, record);
                        }
                        throw error;
                    }
                }
            }
            yield insertTx.commit();
 ...
 */
const deleteBackup = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { backupName } = req.body;
        if (!backupName) {
            res
        ...
        yield fs_2.promises.unlink(backupFilePath);
        res
            .status(200)
 ...
 */
const getTableRecords = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (keyword) {
            ...
            const columnsResult = yield pool.request().query(`
        SELECT COLUMN_NAME 
...
if (process.env.SQL_SERVER_PASSWORD_FILE) {
    try {
        dbPassword = fs_1.default.readFileSync(process.env.SQL_SERVER_PASSWORD_FILE, 'utf-8').trim();
    }
    catch (e) {
...
function validateCourseData(courseMap) {
    for (const [code, courseData] of courseMap.entries()) {
        ...
        ['pre', 'co'].forEach((field) => {
            const requisiteStr = courseData[field];
...
function generateNextId(t, tableName, prefix) {
...
function upsertDegree(t, id, name, totalCredits, isAddon) {
    return __awaiter(this, void 0, void 0, function* () {
        const request = new sql.Request(t);
        ...
        const result = yield request.query('SELECT id FROM Degree WHERE id = @id');
...
function upsertCoursePool(t, poolName) {
    return __awaiter(this, void 0, void 0, function* () {
        const request = new sql.Request(t);
        request.input('name', sql.VarChar, poolName);
        const result = yield request.query('SELECT id FROM CoursePool WHERE name = @name');
        if (result.recordset.length === 0) {
            const newId = yield generateNextId(t, 'CoursePool', 'CP');
...
function upsertCourse(t, code, title, credits, description, offeredIn) {
    return __awaiter(this, void 0, void 0, function* () {
        const request = new sql.Request(t);
        ...
        const result = yield request.query('SELECT code FROM Course WHERE code = @code');
        if (result.recordset.length === 0) {
            yield request.query(`
...
function upsertDegreeXCoursePool(t, degreeId, poolId, creditsRequired) {
    return __awaiter(this, void 0, void 0, function* () {
        const request = new sql.Request(t);
  ...
  `);
        if (result.recordset.length === 0) {
            const newId = yield generateNextId(t, 'DegreeXCoursePool', 'DXCP');
            request.input('newId', sql.VarChar, newId);
            yield request.query(`
...
function upsertCourseXCoursePool(t, courseCode, poolId, groupId) {
    return __awaiter(this, void 0, void 0, function* () {
        const request = new sql.Request(t);
  ...
  `);
        if (result.recordset.length === 0) {
            const newId = yield generateNextId(t, 'CourseXCoursePool', 'CXP');
            request.input('newId', sql.VarChar, newId);
            yield request.query(`
      INSERT INTO CourseXCoursePool (id, coursecode, coursepool, groupId)
      VALUES (@newId, @courseCode, @poolId, @groupId)
    `);
            console.log(`Linked Course ${courseCode} to CoursePool ID: ${poolId}` +
                (groupId ? ` with groupId: ${groupId}` : ''));
            return newId;
        }
        else {
            const cxcpId = result.recordset[0].id;
            console.log(`CourseXCoursePool already exists for Course ${courseCode} in Pool ID: ${poolId}` +
                (groupId ? ` with groupId: ${groupId}` : ''));
            return cxcpId;
        }
    });
}
function upsertRequisite(t, requisite) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const result = yield request.query(`SELECT id FROM Requisite WHERE ${whereClause}`);
        if (result.recordset.length === 0) {
            const newId = yield generateNextId(t, 'Requisite', 'R');
            request.input('newId', sql.VarChar, newId);
            yield request.query(`
    ...
    `);
            console.log(`Inserted Requisite: ${code1}` +
                (code2
                    ? ` -> ${code2}`
            ...
            console.log(`Requisite already exists: ${code1}` +
                (code2
                    ? ` -> ${code2}`
                    : ` with Credits Required: ${creditsRequired}`) +
                ` (${type})` +
...
function getPoolIdByName(t, poolName) {
    return __awaiter(this, void 0, void 0, function* () {
        const request = new sql.Request(t);
        request.input('name', sql.VarChar, poolName);
        const result = yield request.query('SELECT id FROM CoursePool WHERE name = @name');
...
function generateGroupId(degreeId, poolId, groupNumber) {
...
function parseRequisites(code1, requisiteStr, type) {
    if (!requisiteStr)
        return [];
...
function seedSoenDegree() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            // 1. DEFINE PATHS
            const requirementsDir = path_1.default.join(__dirname, '../../course-data/degree-reqs');
            const courseListsDir = path_1.default.join(__dirname, '../../course-data/course-lists/updated_courses');
            // 2. READ ALL REQUIREMENT FILES
            const files = yield (0, promises_1.readdir)(requirementsDir);
            ...
            const dbResult = yield pool.request().query('SELECT DB_NAME() AS dbName');
            ...
            try {
                ...
                for (const [code, courseData] of courseMap.entries()) {
                    ...
                    const cCredits = (_a = courseData.credits) !== null && _a !== void 0 ? _a : 3;
                    const cDesc = (_b = courseData.description) !== null && _b !== void 0 ? _b : `No description for ${code}`;
                    const cOfferedIn = (_d = (_c = courseData.offeredIn) === null || _c === void 0 ? void 0 : _c.join(', ')) !== null && _d !== void 0 ? _d : 'Empty';
                    yield upsertCourse(transaction, code, cTitle, cCredits, cDesc, cOfferedIn);
                }
                ...
                for (const [code, courseData] of courseMap.entries()) {
                    ...
                    for (const preReq of prerequisites) {
                        ...
                        yield upsertRequisite(transaction, preReq);
                    }
                    ...
                    for (const coReq of corequisites) {
                        ...
                        yield upsertRequisite(transaction, coReq);
                    }
                }
            ...
            for (const file of requirementFiles) {
                const filePath = path_1.default.join(requirementsDir, file);
                ...
                try {
                    // Upsert Degree
                    const degreeIdNumber = yield upsertDegree(tx, degreeId, degreeName, totalCredits, isAddon);
                    // Upsert Course Pools and DegreeXCoursePool
                    for (const req of requirements) {
                        const uniquePoolName = req.poolName; // Already includes the degreeId prefix
                        const poolIdNumber = yield upsertCoursePool(tx, uniquePoolName);
                        yield upsertDegreeXCoursePool(tx, degreeIdNumber, poolIdNumber, req.creditsRequired);
                    }
                        ...
                        for (const altLine of alternativeCourses) {
                            ...
                            if (groupCourses.length > 1) {
                                ...
                                for (const courseCode of groupCourses) {
                                    ...
                                    const poolIdNumber = yield getPoolIdByName(tx, req.poolName);
                                    ...
                                    if (isCreditReq) {
                                        ...
                                        yield upsertCourseXCoursePool(tx, upperCode, poolIdNumber, groupId);
                                    }
                                }
                                groupNumber++;
                            }
                                ...
                                if (isCreditReq) {
                                    ...
                                    yield upsertCourseXCoursePool(tx, upperCode, poolIdNumber, null);
                                }
                            }
                        }
