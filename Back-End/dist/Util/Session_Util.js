...
function decryptToken(token) {
    ...
    const decipher = node_crypto_1.default.createDecipheriv(session_algo, session_key, Buffer.from(iv, 'base64'));
...
function createSessionToken(user, salt) {
    ...
    const cipher = node_crypto_1.default.createCipheriv(session_algo, session_key, iv);
...
function refreshSession(token, user) {
    if (!verifySession(token, user))
        return null;
    const salt = decryptToken(token).split('|')[2];
    const new_token = createSessionToken(user, Number.parseInt(salt));
    return new_token;
}
