  ...
  onClose,
    ...
    const poolProgress = useMemo((): PoolProgress[] => {
        ...
        return pools.map((pool) => {
            ...
            const poolCourseIds = new Set(
                pool.courses.map((courseId) => courseId)
            );
    ...
    const { totalTaken, totalRequired } = useMemo(() => ({
        totalTaken: poolProgress.reduce((sum, pool) => sum + pool.creditsTaken, 0),
        totalRequired: poolProgress.reduce((sum, pool) => sum + pool.creditsRequired, 0),
    }), [poolProgress]);
