                ...
                switch (error.category) {
                    case 'MISSING_ENTITY': {
                        const parentContext = error.context.parentId
                            ? ` (parent: ${error.context.parentType || 'unknown'} ${error.context.parentId})`
                            : '';
                        const degreeContext = error.context.degreeId
                            ? ` [degree: ${error.context.degreeId}]`
                            : '';
                        console.error(
                            `    - MISSING ${error.entityType}: ID=${error.entityId} from ${parentContext} of ${degreeContext}`
                        );
                        break;
                    }
                    case 'VALIDATION_FAILURE': {
                        const context = error.context.degreeId || error.context.poolId || 'unknown';
                        const dbPoolInfo = error.context.dbPoolId ? ` [dbPool: ${error.context.dbPoolId}]` : '';
                        console.error(
                            `    - Field '${error.fieldName}': expected '${error.expected}', got '${error.actual}' [context: ${context}] ${dbPoolInfo}`
                        );
                        break;
                    }
                    default: {
